/**
 * Neighboring departments map.
 * For any department, returns the list of adjacent departments
 * whose stations could fall within a ~15km radius search.
 *
 * Built from geographic adjacency of French departments.
 */
...
/** Returns the department code + all neighboring department codes */
export function getDepartmentWithNeighbors(deptCode: string): string[] {
  const neighbors = NEIGHBORS[deptCode] ?? [];
  return [deptCode, ...neighbors];
}
